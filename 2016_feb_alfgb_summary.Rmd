...
cbar=ggplot(means1.sem,aes(ntreat,bio,fill=factor(cut)))+
  ...
  geom_errorbar(aes(ymax=upper,ymin=lower),position=position_dodge(0.5),width=0.2,data=means1.sem,size=1)+
  scale_fill_manual(name="Cutting",breaks=c(1, 2),labels=c("1st", "2nd"),values=c("#888888","#006666"))+
...
ct_bio2=aggregate(ct_bio[,7:10],by=list(ct_bio$plot),FUN=sum)
ct_bio2$finalP=ct_bio2$finalP*2 #finalP expressed on sq m basis
ct_bio3=aggregate(ct_bio[,1:6],by=list(ct_bio$plot),FUN=dumb)
ct_bio=cbind(ct_bio3,ct_bio2)
ct_bio$bio=ct_bio$bio*2/3*0.0040468564 #biomass per square meter conversion -> tons/acre
...
boxplot(ct_bio$bio~ct_bio$treat)
#Plantcount ANOVA
summary(aov((ct_bio$initialP-ct_bio$finalP)~ct_bio$initialP+ct_bio$treat))
#Stem count anova
summary(aov((ct_bio$finalS)~ct_bio$initialP+ct_bio$treat))
#Biomass anova
summary(aov((ct_bio$bio)~ct_bio$initialP+ct_bio$treat))
...
aov_ct1=aov(data=ct_bio,bio~initialP+treat)
...
numobs=data.frame(n=rep(3,7))
watertrt=c("#33cccc","#888888","#006666","#888888","#006666","#888888","#006666")
ctbar=ggplot(means22.sem,aes(treat,mean))
ctbar=ctbar+geom_bar(stat="identity",position="dodge",width=0.5,fill=c("#888888"))+xlab("Timing and water application treatment")
ctbar=ctbar+ylab(bquote('Mean dry alfalfa biomass, '~tons-acre^-1~''))#ylab(bquote('Mean alfalfa biomass,'~g-m^-2~''))
ctbar=ctbar+geom_errorbar(aes(ymax=upper,ymin=lower),position=position_dodge(0.5),width=0.2,data=means22.sem,size=1)+scale_y_continuous(expand=c(0.01,0),limits=c(0,0.6))
...
```{r}
library(gridExtra)
biocbine=grid.arrange(ctbar,cbar,ncol=2)
ggsave("./figures/ct_sv_combinedbio_2015.pdf",biocbine,width = 48, height = 30, units = "cm")
```
